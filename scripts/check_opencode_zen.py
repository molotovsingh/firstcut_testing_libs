#!/usr/bin/env python3
"""
OpenCode Zen Connectivity Test
Tests OpenCode Zen API connectivity and credentials
"""
...
def main():
    """Execute OpenCode Zen connectivity test"""

    log_file = project_root / "docs" / "reports" / "opencode_zen_connection.log"
    ...
    log_result("OpenCode Zen Connectivity Test", log_file)
    log_result("=" * 80, log_file)
    ...
    model = os.getenv("OPENCODEZEN_MODEL", "opencode-zen/legal-extractor")

    if api_key:
        log_result(f"✅ OPENCODEZEN_API_KEY found (length: {len(api_key)} chars)", log_file)
        log_result(f"   Base URL: {base_url}", log_file)
        log_result(f"   Model: {model}", log_file)
    else:
        log_result("⚠️ OPENCODEZEN_API_KEY not found in environment", log_file)
        log_result("   To test OpenCode Zen, set OPENCODEZEN_API_KEY in .env", log_file)
    ...
    log_result("\n3. Loading OpenCode Zen configuration...", log_file)
    try:
    ...
    # Initialize adapter
    log_result("\n4. Initializing OpenCode Zen adapter...", log_file)
    try:
    ...
    if is_available:
        log_result("✅ OpenCode Zen adapter is available", log_file)
    else:
        log_result("❌ OpenCode Zen adapter reports unavailable", log_file)
    ...
    test_text = """
    This Employment Agreement was signed on February 10, 2024, between Employer Corp
    and Employee Jane Doe. Employment commences on March 1, 2024 with an annual salary
    of $120,000 payable bi-weekly. Confidentiality obligations survive termination.
    """.strip()

    test_metadata = {
        "file_path": "test_opencode_zen_connectivity.txt",
        "document_name": "test_opencode_zen_connectivity.txt"
    }

    try:
        log_result(f"   Test text length: {len(test_text)} chars", log_file)
        log_result("   Calling OpenCode Zen API via extract_events()...", log_file)
        ...
        log_result("OpenCode Zen API is reachable and functional", log_file)
        log_result("=" * 80, log_file)
        return 0
